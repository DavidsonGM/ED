...
int remove_valor(lse_int **lista, int valor){
    if(*lista == NULL)
        return 0;
    lse_int *ptr_aux = *lista, *aux = *lista;
    int i = 0, j;
    while(ptr_aux->prox != NULL){
        if(ptr_aux->dado == valor){
            aux = ptr_aux->prox;
            ptr_aux = *lista;
            for(j = 0; j < i - 1; j++)
                ptr_aux = ptr_aux->prox;
...
void troca(lse_int **lista,int atual,int novo){
    if(*lista != NULL){
        lse_int *ptr_aux = *lista;
        int i = 0, j;
        while(ptr_aux->prox != NULL){
            if(ptr_aux->dado == atual){
                for(j = 0; j < i - 1; j++)
                    ptr_aux = ptr_aux->prox;
                    (*ptr_aux).dado = novo;
                break;
...
int ocorrencias(lse_int **lista, int valor){
    if(*lista != NULL){
        lse_int *ptr_aux = *lista;
        int aux = 0;
        while(ptr_aux->prox != NULL){
